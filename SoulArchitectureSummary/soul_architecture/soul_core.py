    ...
    async def execute(
        ...
        try:
            ...
            if not skip_security:
                default_required_permissions = ["ai:use"] if context.granted_permissions else []
                security_context = {
                    "user_id": context.user_id,
                    "auth_token": context.input_metadata.get("auth_token") or f"session:{context.session_id}",
                    "content": context.input_content,
                    "granted_permissions": context.granted_permissions,
                    "required_permissions": context.input_metadata.get(
                        "required_permissions",
                        default_required_permissions,
                    ),
                    "tool_whitelist": context.input_metadata.get("tool_whitelist", ["default_executor"]),
                    "tool_name": context.input_metadata.get("tool_name", "default_executor"),
                    "sql": context.input_metadata.get("sql", "SELECT 1"),
                }
            ...
            if not executor:
                fallback_executor = self._executors.get(ExecutionMode.HYBRID)
                if fallback_executor:
                    logger.warning(
                        f"模式 {decision.mode.value} 無專用執行器，回退到 hybrid 執行器"
                    )
