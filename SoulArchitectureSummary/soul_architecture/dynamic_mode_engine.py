...
class SensitivityAnalyzer:
    ...
    DEFAULT_RULES = [
        # 金融類 (高敏感)
        SensitivityRule(r"密碼|password|密鑰|private.?key", 0.95, "credential"),
        SensitivityRule(r"信用卡|credit.?card|CVV|\b\d{16}\b", 0.9, "financial"),
        SensitivityRule(r"轉帳|匯款|transfer|balance", 0.85, "financial"),
        SensitivityRule(r"銀行帳號|account.?number", 0.8, "financial"),
        
        # 個人身份類 (中高敏感)
        SensitivityRule(r"身份證|ID.?card|身分證", 0.95, "identity"),
        SensitivityRule(r"護照|passport", 0.9, "identity"),
        SensitivityRule(r"手機號|phone|\b\d{11}\b", 0.7, "identity"),
        SensitivityRule(r"地址|address|住址", 0.6, "identity"),
        SensitivityRule(r"姓名|name", 0.5, "identity"),
        
        # 系統操作類 (中高敏感)
        SensitivityRule(r"刪除|delete|drop|truncate", 0.85, "system"),
        SensitivityRule(r"修改|update|alter", 0.75, "system"),
        SensitivityRule(r"執行|exec|eval|system", 0.7, "system"),
        SensitivityRule(r"rm\s+-rf|chmod\s+777", 0.95, "system"),

        # 一般分析任務（中敏感）
        SensitivityRule(r"分析|分析報告|數據|数据|dataset|analytics", 0.5, "analysis"),
        
        # 商業機密類
        SensitivityRule(r"營業額|revenue|profit", 0.75, "business"),
        SensitivityRule(r"客戶名單|customer.?list", 0.8, "business"),
        SensitivityRule(r"合約|contract|agreement", 0.6, "business"),
    ]
    ...
    def analyze(self, content: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        for rule in self.rules:
            if self._matches(content_lower, rule):
    ...
    def _matches(self, content: str, rule: SensitivityRule) -> bool:
        """檢查內容是否匹配規則"""
        should_use_regex = rule.is_regex or bool(
            re.search(r"[|\\\\.^$*+?()\[\]{}]", rule.pattern)
        )
